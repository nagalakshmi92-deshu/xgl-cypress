...
describe('Verification of Reports (Smoke)', () => {
    ...
    it('Run SQL commands report Test', () => {
        cy.fixture('Configurations.json').then(function (data) {
            ...
            getIframeBody().find('.xg-page-content').find('[href="#/job/ad-copy/300015"]', { timeout: 20000 }).click()
            cy.log('Clicked on Run SQL Command successfully')

            // Step 4: Click on submit 
            cy.getIframeBody('//iframe[@class="runReportIframe ng-scope"]').find('.xg-page-header').find('.tabs.is-toggle').find('li:nth-of-type(3) > a > span:nth-of-type(2)').click().click({ force: true });
            cy.log('Clicked on schedule options button successfully')

            //Input data into description
            this.reportName = 'Ad Copy' + generateString(2)

            cy.getIframeBody('//iframe[@class="runReportIframe ng-scope"]').find('.xg-page-content').find('input[class="input"][name="xgj_jobDescription"]', { timeout: 10000 }).clear().type(this.reportName)
            cy.log('Added description is =' + this.reportName)
            cy.wait(5000)
            ...
            //search with respect to description 
            cy.xpath('//div[contains(.,"Description")]/following-sibling::input', { log: false }).type(this.reportName + '{enter}')
            // cy.wait(5000)
            ...
            cy.xpath("//div[@class='grid-canvas']/div", { timeout: 600000 }).each((ee, index, list) => {
                ...
                if (c1 == this.reportName) {
                    ...
                    cy.log('Successfully! record status changed !')
                }
            })
