...
describe('Verification of DST: When the order line is ordered Quantity Per Day (Retail or Threshold Billing Mode)', () => {
    ...
    it('XGLT-6791_DST When the order line is an Orbit (Retail or Threshold Billing Mode)', () => {
           ...
           rt.locator_Click_on_Appprove_Orderline().click({ force: true })
            ...
            cy.xpath("//div[@class='slick-header-columns']/div").each((ee,index,list)=>{
                if(ee.text().includes('Date/Time'))
                {
                    ...
                    cy.get("div.grid-canvas div div:nth-child("+index1+")").each((ee, index1, list) => {
                        cy.log('index:'+index)
                        var e1 = ee.text().trim().split('')
                        cy.log(e1)
                        var e2 = e1[1].split(':')
                        if (!(e2[0] == 2)) {
                            cy.log(index + ' Dates time are day light saving dates')
                        }
                        else {
                            cy.log(index + ' Dates time are not day light saving dates')
                            assert.fail()
                        }
                    })
                }
